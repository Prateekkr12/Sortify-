        ...
        {isOpen && (
          ...
          >
            <div className="p-4">
              ...
              {/* Add New Category Form - Disabled: Only predefined categories are available */}
              <div className="mb-4">
                <div className="flex gap-3 opacity-50">
                  <input
                    type="text"
                    disabled
                    placeholder="Only predefined categories are available"
                    className="flex-1 px-3 py-2 bg-white/60 border border-white/50 rounded-xl text-slate-500 placeholder-slate-400 min-w-0 cursor-not-allowed"
                  />
                  <button
                    type="button"
                    disabled
                    className="px-3 py-2 bg-gray-300 text-gray-500 rounded-xl font-semibold cursor-not-allowed flex items-center gap-1.5 whitespace-nowrap min-w-[80px] justify-center"
                    title="Only predefined categories are available"
                  >
                    <span className="text-sm">Add</span>
                  </button>
                </div>
                <p className="text-xs text-slate-500 mt-1 ml-1">Categories are fixed and cannot be added or removed</p>
              </div>
                  ...
                  categories.map((category) => (
                    ...
                    >
                      ...
                      <div className="flex items-start gap-1 flex-shrink-0 pt-1">
                        ...
                        <button
                          onClick={() => !category.isDefault && handleDeleteCategory(category.id, category.name)}
                          disabled={category.isDefault === true}
                          className={`p-1 rounded-lg transition-colors ${
                            category.isDefault === true 
                              ? 'text-gray-400 cursor-not-allowed' 
                              : 'text-red-600 hover:bg-red-100'
                          }`}
                          title={category.isDefault === true ? 'Cannot delete default categories' : 'Delete category'}
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
